<template>
  <div class="app-container">
    <el-form :inline="true" class="demo-form-inline">
      <el-form-item label="电子零件类别">
        <el-input v-model="dataVo.categoryName" placeholder="请输入电子零件类别" />
      </el-form-item>
    ...
    >
      ...
      <el-table-column prop="categoryName" label="电子零件类别" />
    ...
    >
      ...
      <el-form :model="formData" label-position="left" label-width="100px">
        <el-form-item label="电子零件类别">
          <el-input v-model="formData.categoryName" placeholder="请填写电子零件类别"/>
        </el-form-item>
      </el-form>
...
<script>
import {pageQuery, update, insert, deleteById} from '@/api/category'
export default {
  data() { // 定义变量和初始值
    return {
      list: [], // 查询之后接口返回集合
  ...
  methods: {
    ...
    updateAndSave(row) {
      if (row == null) {
        this.title = '新增电子零件分类信息';
        this.formData = {
          categoryName:'',
          categoryDetail:''
        }
      } else {
        this.title = '修改电子零件分类信息';
        this.formData = row
      }
